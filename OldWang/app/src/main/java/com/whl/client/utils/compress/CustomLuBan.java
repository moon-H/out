package com.whl.client.utils.compress;
...
public class CustomLuBan {
    ...
    private static volatile CustomLuBan INSTANCE;
    ...
    private CustomLuBan(File cacheDir) {
        mCacheDir = cacheDir;
    }
    ...
    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, CustomLuBan.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    public static CustomLuBan get(Context context) {
        if (INSTANCE == null)
            INSTANCE = new CustomLuBan(CustomLuBan.getPhotoCacheDir(context));
        return INSTANCE;
    }

    public CustomLuBan launch() {
        ...
        BizApplication.getInstance().getThreadPool().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    File file1 = thirdCompress(mFile);
                    if (compressListener != null)
                        compressListener.onSuccess(file1);
                } catch (Exception e) {
                    if (compressListener != null)
    ...
    public File syncCompress() throws Exception {
        Preconditions.checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
        return thirdCompress(mFile);
    }

    public CustomLuBan load(File file) {
        mFile = file;
        return this;
    }

    public CustomLuBan setCompressListener(OnCompressListener listener) {
        compressListener = listener;
        return this;
    }

    public CustomLuBan putGear(int gear) {
        this.gear = gear;
        return this;
    }
     ...
     * @deprecated
     */
    public CustomLuBan setFilename(String filename) {
    ...
    private File thirdCompress(File file) throws Exception {
    ...
    private File firstCompress(File file) throws Exception {
        int minSize = 60;
        int longSide = 720;
        int shortSide = 1280;
     ...
     */
    private int getImageSpinAngle(String path) throws Exception {
        int degree = 0;
