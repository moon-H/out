...
public class DividerGridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerGridItemDecoration(Context context, int horizontalSpace) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, State state) {
        ...
        MLog.d("123", "space =" + space);
        int itemPosition = parent.getChildAdapterPosition(view); // item position
        int spanCount = getSpanCount(parent);
        int childCount = parent.getAdapter().getItemCount();
        if (isLastRaw(parent, itemPosition, spanCount, childCount))// 如果是最后一行，则不需要绘制底部
        {
            MLog.d("123", "is last raw=" + itemPosition);
            outRect.set(space, space, 0, 0);
        } else if (isLastColum(parent, itemPosition, spanCount, childCount))// 如果是最后一列，则不需要绘制右边
        {
            MLog.d("123", "is last column= " + itemPosition);
            outRect.set(space, space, 0, 0);
        } else if (isFirstColumn(parent, itemPosition, spanCount, childCount)) {
            MLog.d("123", "isFirstColumn= " + itemPosition);

            outRect.set(0, space, 0, 0);
        } else {
            MLog.d("123", "is common=" + itemPosition);
            outRect.set(space, space, 0, 0);
        }
