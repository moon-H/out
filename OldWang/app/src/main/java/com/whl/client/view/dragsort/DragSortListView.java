...
package com.whl.client.view.dragsort;
...
public class DragSortListView extends ListView {
    ...
    /**
     * The OrderView that floats above the ListView and represents
     * the dragged item.
     */
    private View mFloatView;

    /**
     * The float OrderView location. First based on touch location
    ...
    /**
     * The middle (in the y-direction) of the floating OrderView.
     */
    private int mFloatViewMid;

    /**
     * Flag to make sure float OrderView isn't measured twice
     */
    ...
    /**
     * Transparency for the floating OrderView (XML attribute).
     */
     ...
     * OrderView. If dropped, the dragged item will land in this position.
     */
    private int mFloatPos;

    /**
     * The first expanded ListView position that helps represent
     * the drop slot tracking the floating OrderView.
     */
    private int mFirstExpPos;

    /**
     * The second expanded ListView position that helps represent
     * the drop slot tracking the floating OrderView. This can equal
    ...
    /**
     * A listener that receives callbacks whenever the floating OrderView
     * hovers over a new position.
     */
    private DragListener mDragListener;

    /**
     * A listener that receives a callback when the floating OrderView
     * is dropped.
     */
    ...
    /**
     * Height of the floating OrderView. Stored for the purpose of
     ...
     * where scroll speed increases linearly as the floating OrderView
    ...
    /**
     * Drag flag bit. Floating OrderView can move in the positive
     * x direction.
     */
    public final static int DRAG_POS_X = 0x1;

    /**
     * Drag flag bit. Floating OrderView can move in the negative
     * x direction.
     */
     ...
     * that, if enabled, the floating OrderView can be dragged below its starting
     ...
     * Drag flag bit. Floating OrderView can move in the negative
     * y direction. This is subtle. What this actually means is
     * that the floating OrderView can be dragged above its starting
     ...
     * floating OrderView. See flags above.
     */
    ...
    /**
     * Let the user customize the floating OrderView.
     */
     ...
     * OrderView must be to initiate the slide.
     */
     ...
     * item OrderView given by the user inside another OrderView (currenly
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            // OrderView needs to be measured if measurement is required.
    ...
    private int getItemHeight(int position) {
        ...
        if (v != null) {
            // item is onscreen, just get the height of the OrderView
            return v.getHeight();
        } else {
     ...
     * floating OrderView (treated as horizontal line) is dropped
    ...
    private int getShuffleEdge(int position, int top) {
        ...
        // first calculate top of item given that floating OrderView is
    ...
    /**
     * Centers floating OrderView under touch point.
     */
    ...
    /**
     * Centers floating OrderView over drop slot before destroying.
     */
     ...
     * the floating OrderView from the FloatViewManager.
     *
     * @param position  Item to drag.
     * @param dragFlags Flags that restrict some movements of the
     *                  floating OrderView. For example, set <code>dragFlags |=
     *                  ~{@link #DRAG_NEG_X}</code> to allow dragging the floating
     *                  OrderView in all directions except off the screen to the left.
     * @param deltaX    Offset in x of the touch coordinate from the
     *                  left edge of the floating OrderView (i.e. touch-x minus float OrderView
     *                  left).
     * @param deltaY    Offset in y of the touch coordinate from the
     *                  top edge of the floating OrderView (i.e. touch-y minus float OrderView
     *                  top).
     ...
     * or the FloatViewManager returns a null OrderView.
     */
     ...
     *
     * @param position  Item to drag.
     * @param floatView Floating OrderView.
     * @param dragFlags Flags that restrict some movements of the
     *                  floating OrderView. For example, set <code>dragFlags |=
     *                  ~{@link #DRAG_NEG_X}</code> to allow dragging the floating
     *                  OrderView in all directions except off the screen to the left.
    ...
    /**
     * Sets float OrderView location based on suggested values and
    ...
    private void updateFloatView() {
        ...
        // bottom limit is top of first footer OrderView or
    ...
    /**
     * Interface for customization of the floating OrderView appearance
    ...
    public interface FloatViewManager {
        /**
         * Return the floating OrderView for item at <code>position</code>.
         * DragSortListView will measure and layout this OrderView for you,
         * so feel free to just inflate it. You can help DSLV by
         * setting some {@link ViewGroup.LayoutParams} on this OrderView;
         ...
         * @return The OrderView you wish to display as the floating OrderView.
         */
        public View onCreateFloatView(int position);

        /**
         * Called whenever the floating OrderView is dragged. Float OrderView
         * properties can be changed here. Also, the upcoming location
         * of the float OrderView can be altered by setting
         * <code>location.x</code> and <code>location.y</code>.
         *
         * @param floatView     The floating OrderView.
         * @param location      The location (top-left; relative to DSLV
         *                      top-left) at which the float
         *                      OrderView would like to appear, given the current touch location
        ...
        /**
         * Called when the float OrderView is dropped; lets you perform
         * any necessary cleanup. The internal DSLV floating OrderView
         * reference is set to null immediately after this is called.
         *
         * @param floatView The floating OrderView passed to
    ...
    private class DragScroller implements Runnable {
        ...
        public void run() {
            ...
            // dy is change in OrderView position of a list item; i.e. positive dy
            // means user is scrolling up (list item moves down the screen,
            // remember
            // y=0 is at top of OrderView).
            ...
            // scroll means relative float OrderView movement
